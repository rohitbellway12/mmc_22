                            ...
                            <tbody id="service-edit-tbody">
                                ...
                                @foreach ($booking->detail as $key => $detail)
                                    <tr id="service-row--{{ $detail?->variant_key }}">
                                        ...
                                        <td id="service-cost-{{ $detail?->variant_key }}">{{ $detail->service_cost }}
                                        </td>
                                        <td>
                                            <input type="number" min="1" name="qty[]"
                                                class="form-control qty-width" id="qty-{{ $detail?->variant_key }}"
                                                value="{{ $detail->quantity }}"
                                                oninput="this.value = this.value.replace(/[^0-9]/g, '');" readonly>
                                        </td>
                                        <td id="discount-amount-{{ $detail?->variant_key }}">
                                            {{ $detail->discount_amount }}</td>
                                        <td id="total-cost-{{ $detail?->variant_key }}">{{ $detail->total_cost }}</td>
                                        <td>
                                            <div class="d-flex justify-content-center">
                                                <span
                                                    class="material-icons text-danger cursor-pointer remove-service-row"
