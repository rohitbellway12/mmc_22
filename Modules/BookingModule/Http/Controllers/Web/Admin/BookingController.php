...
class BookingController extends Controller
{
    ...
    public function details($id, Request $request): Renderable|RedirectResponse
    {
        ...
        $carBooking = CarBooking::where('booking_id', $id)->first();
        if ($carBooking) {
            return redirect()->route('admin.car.booking.details', [$carBooking->id]);
        }
        ...
        if ($request->web_page == 'details') {
            ...
            if (!$booking) {
                ...
                return redirect(route('admin.booking.list', ['booking_status' => 'pending']));
            }
            ...
            $services = $category
                ? Service::select('id', 'name')->where('category_id', $category->id)->get()
                : collect();
            ...
            return view('bookingmodule::admin.booking.details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'providers', 'sort_by', 'currentlyAssignProvider'));
            ...
            $sort_by = 'default';
            return view('bookingmodule::admin.booking.status', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'services', 'providers', 'zones', 'sort_by', 'currentlyAssignProvider'));
        }
    ...
    public function repeatDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        if ($webPage == 'details') {
            return view('bookingmodule::admin.booking.repeat-booking-details', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'providers', 'sort_by'));

        } elseif ($webPage == 'service_log') {
            return view('bookingmodule::admin.booking.service-log', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'providers', 'sort_by'));
    ...
    public function repeatSingleDetails($id, Request $request): Renderable|RedirectResponse
    {
        ...
        if ($request->web_page == 'details') {
            return view('bookingmodule::admin.booking.rebooking-ongoing', compact('zoneCenter', 'currentZone', 'centerLat', 'centerLng', 'area', 'booking', 'servicemen', 'webPage', 'customerAddress', 'services', 'zones', 'category', 'providers', 'sort_by'));

        } elseif ($request->web_page == 'status') {
            return view('bookingmodule::admin.booking.repeat-status', compact('booking', 'webPage', 'servicemen', 'customerAddress', 'category', 'services', 'providers', 'zones', 'sort_by'));
        }
    }
        ...
        if (isset($booking)) {
            ...
            if (isset($booking->provider_id)) {
                ...
                foreach ($providers as $provider) {
                    ...
                    if (!is_null($fcmToken) && (!business_config('suspend_on_exceed_cash_limit_provider', 'provider_config')->live_values || $provider?->is_suspended == 0))
                        device_notification($fcmToken, $title, null, null, $booking->id, 'booking');
                }
            }
            return response()->json(response_formatter(DEFAULT_STATUS_UPDATE_200), 200);
