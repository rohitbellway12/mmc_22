...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeat $bookingRepeat, BookingRepeatDetails $bookingRepeatDetail, BookingRepeatHistory $bookingRepeatHistory, BookingIgnore $bookingIgnore, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, SubscribedService $subscribedService, Category $category, Zone $zone, Serviceman $serviceman, Provider $provider, SubscribedService $subscribed_service, User $user, UserAddress $userAddress, BookingDetail $bookingDetail)
    {
        ...
        try {
            $this->subscribed_categories = $subscribedService->where(['provider_id' => auth()->user()->provider->id])
                ->where(['is_subscribed' => 1])->pluck('category_id')->toArray();
        } catch (\Exception $exception) {
            $this->subscribed_categories = $subscribedService->pluck('category_id')->toArray();
        }
    }
    ...
    public function index(Request $request): Renderable
    {
        ...
        if ($bookingStatus == 'pending') {
            $this->booking
                ->whereIn('category_id', $this->subscribed_categories)
    ...
    public function checkBooking($id): void
    {
        ...
        $this->booking->where('id', $id)
            ->where(function ($query) use ($providerId) {
                $query->whereIn('category_id', $this->subscribed_categories)
    ...
    public function details($id, Request $request): View|Factory|RedirectResponse|Application
    {
        ...
        // If this booking is a mirrored car booking, redirect to car booking details page
        $carBooking = CarBooking::where('booking_id', $id)->first();
        if ($carBooking) {
            return redirect()->route('provider.car.booking.details', [$carBooking->id]);
        }
        ...
        if ($request->web_page == 'details') {
            ...
            return view('bookingmodule::provider.booking.details', compact('booking', 'servicemen', 'webPage', 'customerAddresses', 'category', 'services', 'customerAddress', 'zones'));
            ...
            $customerAddress = $this->userAddress->find($booking['service_address_id']);
            $zones = Zone::ofStatus(1)->get();
            return view('bookingmodule::provider.booking.status', compact('booking', 'servicemen', 'webPage', 'customerAddress', 'category', 'zones', 'services'));
        }
    }
