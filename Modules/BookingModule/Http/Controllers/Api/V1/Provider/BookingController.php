...
class BookingController extends Controller
{
    ...
    public function __construct(Booking $booking, BookingRepeat $bookingRepeat, BookingRepeatDetails $bookingRepeatDetail, BookingStatusHistory $bookingStatusHistory, BookingScheduleHistory $bookingScheduleHistory, SubscribedService $subscribedService, BookingDetail $bookingDetail, BookingIgnore $bookingIgnore, BookingRepeatHistory $bookingRepeatHistory)
    {
        ...
        try {
            ...
            if ($provider) {
                $this->subscribedCategories = $subscribedService->where(['provider_id' => $provider->id])
                    ->where(['is_subscribed' => 1])->pluck('category_id')->toArray();
            } else {
                $this->subscribedCategories = [];
            }
        } catch (\Exception $exception) {
            $this->subscribedCategories = $subscribedService->pluck('category_id')->toArray();
        }
    }
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $bookings_count['pending'] = $this->booking
            ...
            ->when(in_array($request['service_type'], ['repeat', 'regular']), function ($query) use ($request) {
        ...
        $bookings_count['accepted'] = $this->booking
            ->providerAcceptedBookings($providerId, $maxBookingAmount)
            ->whereDoesntHave('ignores', function ($query) use ($providerId) {
                $query->where('provider_id', $providerId);
            })
            ->when(in_array($request['service_type'], ['repeat', 'regular']), function ($query) use ($request) {
    ...
    public function download(Request $request): JsonResponse
    {
        ...
        $providerId = $provider->id;
        $bookings = $this->booking->where('provider_id', $providerId)
    ...
    public function show(Request $request, string $id): JsonResponse
    {
        ...
        $booking = $this->booking->with([
            ...
            'slotBooking.timeSlot',
            'questionAnswers.question',
            'repeat.detail.service' => fn($query) => $query->withTrashed(),
            'repeat.repeatHistories',
            'carHireBooking.car'
        ])->where(function ($query) use ($providerId, $request) {
            return $query->where(function ($query) use ($providerId) {
                $query->where('provider_id', $providerId)
                    ->orWhereHas('repeat', function ($subQuery) use ($providerId) {
                        $subQuery->where('provider_id', $providerId);
                    });
    ...
    public function singleDetails(Request $request, string $id): JsonResponse
    {
        ...
        $booking = $this->bookingRepeat->with([
            ...
            return $query->where('provider_id', $providerId)->orWhereNull('provider_id');
        })->where(['id' => $id])->first();
    ...
    public function requestAccept(Request $request, string $bookingId): JsonResponse
    {
        ...
        $booking = $this->booking->where('id', $bookingId)
            ->where(function ($query) use ($providerId) {
                $query->whereIn('category_id', $this->subscribedCategories)
        ...
        if (isset($booking)) {
            ...
            DB::transaction(function () use ($bookingStatusHistory, $booking, $request, $providerId) {
                $booking->save();
                ...
                // Sync car booking status if exists
                if ($booking->carHireBooking) {
                    $booking->carHireBooking->booking_status = 'accepted';
                    $booking->carHireBooking->save();
                }
            });
    ...
    public function requestIgnore(Request $request, string $bookingId): JsonResponse
    {
        ...
        if (isset($booking)) {
            ...
            $bookingIgnore->reason = $request->reason ?? $request->ignore_reason ?? $request->input('reason');
    ...
    public function statusUpdate(Request $request, string $bookingId): JsonResponse
    {
        ...
        $providerId = $request->user()->provider->id;
        $booking = $this->booking->where('id', $bookingId)->where(function ($query) use ($providerId) {
            $query->whereIn('category_id', $this->subscribedCategories)
                ->whereNull('provider_id')
                ->orWhere('provider_id', $providerId);
        })->first();
        ...
        if (isset($booking)) {
            ...
            if ($booking->isDirty('booking_status')) {
                DB::transaction(function () use ($bookingStatusHistory, $booking, $request) {
                    ...
                    if ($booking->repeat->isNotEmpty()) {
                        foreach ($booking->repeat as $repeat) {
                            if (in_array($repeat->booking_status, ['pending', 'accepted', 'ongoing'])) {
                                $repeat->booking_status = $request['booking_status'];
                                $repeat->save();
                                ...
                                $repeatBookingStatusHistory->booking_repeat_id = $repeat->id;
                                $repeatBookingStatusHistory->changed_by = $request->user()->id;
                                $repeatBookingStatusHistory->booking_status = $request['booking_status'];
                                $repeatBookingStatusHistory->save();
                            }
                        }
                    }
                    ...
                    // Sync car booking status if exists
                    if ($booking->carHireBooking) {
                        $booking->carHireBooking->booking_status = $request['booking_status'];
                        $booking->carHireBooking->save();
                    }
                });
    ...
    public function singleBookingStatusUpdate(Request $request, string $repeatId): JsonResponse
    {
        ...
        $booking = $this->bookingRepeat->where('id', $repeatId)->where(function ($query) use ($providerId) {
            return $query->where('provider_id', $providerId)->orWhereNull('provider_id');
        })->first();
        ...
        if (isset($booking)) {
            ...
            if ($booking->isDirty('booking_status')) {
                DB::transaction(function () use ($bookingStatusHistory, $booking) {
                    ...
                    if ($booking->booking) {
                        $booking->booking->is_checked = 1;
                        if (!$booking->booking->provider_id) {
                            $booking->booking->provider_id = $booking->provider_id;
                        }
    ...
    public function assignServiceman(Request $request, string $bookingId): JsonResponse
    {
        ...
        if ($request->booking_type == 'repeat') {
            $booking = $this->bookingRepeat->where('id', $bookingId)->where('provider_id', $providerId)->first();
        } else {
    ...
    public function getServiceInfo(Request $request): JsonResponse
    {
        ...
        foreach (json_decode($request['service_info'], true) as $item) {
            ...
            if (!isset($service))
                return response()->json(response_formatter(DEFAULT_404, $data), 404);
    ...
    public function updateBooking(Request $request): JsonResponse
    {
        ...
        if (!is_null($request['payment_status']))
            $booking->is_paid = $request['payment_status'];
        if (!is_null($request['serviceman_id']))
            $booking->serviceman_id = $request['serviceman_id'];
        ...
        if ($providerEditAccess && $serviceInfoValidated) {
            ...
            if ($request['next_all_booking_change'] == 1) {
                ...
                if ($totalOldQuantity != $totalNewQuantity) {
                    ...
                    $bookingRepeatHistory->new_quantity = $totalNewQuantity;
                    $bookingRepeatHistory->is_multiple = $request['next_all_booking_change'] ? 1 : 0;
                    $bookingRepeatHistory->readable_id = $sourceRepeatBooking->readable_id;
                    $bookingRepeatHistory->log_details = json_encode($updateQuantity);
