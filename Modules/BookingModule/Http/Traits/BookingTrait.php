...
trait BookingTrait
{
     ...
     * @return array|string[]
     */
    public function placeBookingRequest($userId, $request, $transactionId, $newUserInfo = null, int $isGuest = 0, array $additionalData = []): array
    {
        $oldUserId = $userId;
        ...
        foreach ($cartData->pluck('sub_category_id')->unique() as $subCategory) {
            ...
            DB::transaction(function () use ($subCategory, $booking, $transactionId, $request, $cartData, $isGuest, $isPartials, $customerWalletBalance, &$userId, // Pass by reference
                ...
                $emergencyCharge = 0;
                if (isset($additionalData['booking_type']) && $additionalData['booking_type'] == 'emergency') {
                    $emergencyCharge = 100; // Fixed charge as per requirement
                    $extraFee += $emergencyCharge;
                }
                ...
                // New Fields
                $booking->booking_type = $additionalData['booking_type'] ?? 'normal';
                $booking->emergency_charge = $emergencyCharge;
                $booking->damage_description = $additionalData['damage_description'] ?? null;
                $booking->car_registration_number = $additionalData['car_registration_number'] ?? null;
                $booking->car_model = $additionalData['car_model'] ?? null;
                $booking->car_manufacture_year = $additionalData['car_manufacture_year'] ?? null;
                $booking->car_color = $additionalData['car_color'] ?? null;
                $booking->special_conditions = $additionalData['special_conditions'] ?? null;
                $booking->notes = $additionalData['notes'] ?? null;
                $booking->postcode = $additionalData['postcode'] ?? null;

                if (isset($additionalData['car_image']) && !is_null($additionalData['car_image'])) {
                    $image_name = file_uploader('booking/', $additionalData['car_image']->getClientOriginalExtension(), $additionalData['car_image']);
                    $booking->evidence_photos = [$image_name];
                }

                $booking->save();

                // Slot Booking
                if (isset($additionalData['selected_slot_id'])) {
                    \Modules\BookingModule\Entities\SlotBooking::create([
                        'booking_id' => $booking->id,
                        'provider_time_slot_id' => $additionalData['selected_slot_id'],
                        'booking_date' => date('Y-m-d', strtotime($request['service_schedule']))
                    ]);
                }

                // Question Answers
                if (isset($additionalData['answers']) && is_array($additionalData['answers'])) {
                    foreach ($additionalData['answers'] as $questionId => $answer) {
                ...
                foreach ($cartData->all() as $datum) {
                    ...
                    $detail->tyre_id = $datum['tyre_id'] ?? null;
    ...
    public function placeRepeatBookingRequest($userId, $request, $transactionId, $newUserInfo = null, int $isGuest = 0): array
    {
        ...
        foreach ($cartData->pluck('sub_category_id')->unique() as $subCategory) {
            ...
            DB::transaction(function () use ($subCategory, $booking, $transactionId, $request, $cartData, $isGuest, &$userId, // Pass by reference
                ...
                $booking->damage_description = $request->damage_description;
                $booking->car_registration_number = $request->car_registration_number;
                $booking->car_model = $request->car_model;
                $booking->car_color = $request->car_color;
                $booking->special_conditions = $request->special_conditions;
                ...
                foreach ($repeatBookingSchedule as $index => $repeat) {
                    ...
                    foreach ($cartData as $datum) {
                        ...
                        $repeatBookingDetails->tyre_id = $datum['tyre_id'] ?? null;
