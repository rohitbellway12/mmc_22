...
class CartController extends Controller
{
    ...
    public function __construct(Cart $cart, Service $service, Variation $variation, User $user, Provider $provider, Guest $guest, Request $request, Booking $booking, Coupon $coupon)
    {
        ...
        $this->customerUserId = $this->isCustomerLoggedIn ? auth('api')->id() : $request['guest_id'];
    }
    ...
    public function addToCart(Request $request): JsonResponse
    {
        ...
        if (empty($serviceIds)) {
            return response()->json(response_formatter(DEFAULT_400, null, 'service_id or service_ids is required'), 400);
        }
        ...
        if ($request['provider_id']) {
            if (!$request->has('is_terms_accepted') || $request['is_terms_accepted'] != 1) {
                return response()->json(response_formatter([
                    'response_code' => 'terms_not_accepted_400',
                    'message' => 'Please accept the provider\'s terms and conditions to proceed.',
                    'content' => []
                ]), 400);
            }
        ...
        foreach ($serviceIds as $sId) {
            $this->addedToCartUpdate($customerUserId, $sId, !$this->isCustomerLoggedIn);

            $currentCategoryId = $request->category_id;
            $currentSubCategoryId = $request->sub_category_id;
            $currentVariantKey = $request->variant_key;

            $service = $this->service->find($sId);
            if (!$service)
                continue;

            if (!$currentCategoryId || !$currentSubCategoryId || !$currentVariantKey) {
                $currentCategoryId = $currentCategoryId ?? $service->category_id;
                $currentSubCategoryId = $currentSubCategoryId ?? $service->sub_category_id;

                if (!$currentVariantKey) {
                    $variation = $this->variation
                        ->where(['zone_id' => Config::get('zone_id'), 'service_id' => $sId])
                        ->first();
                    if ($variation) {
                        $currentVariantKey = $variation->variant_key;
                    }
                }
            }
            ...
            $variation = $this->variation
                ->where(['zone_id' => Config::get('zone_id'), 'service_id' => $sId])
                ->where(['variant_key' => $currentVariantKey ?? 'default'])
                ->first();

            if (isset($variation) || isset($service)) {
                $checkCart = $this->cart->where([
                    'service_id' => $sId,
                    'variant_key' => $currentVariantKey ?? ($variation->variant_key ?? 'default'),
                ...
                $price = $variation->price ?? ($service->price ?? 0);
                $providerId = $request['provider_id'];

                // Tyre Price Logic
                if ($request->has('tyre_id') && !empty($request['tyre_id']) && $sId == $request->service_id) {
                    $tyre = \Modules\ServiceManagement\Entities\Tyre::find($request['tyre_id']);
                    if ($tyre) {
                        $price = $tyre->price;
                        $providerId = $tyre->provider_id;
                    }
                }

                if ($providerId && !$request->has('tyre_id')) {
                    // Check for variation override
                    $customPrice = null;
                    if (isset($variation)) {
                        $customPrice = ProviderServicePrice::where('provider_id', $providerId)
                    ...
                    if ($customPrice) {
                        ...
                        // Check for service-level override in subscribed_services
                        $subscribedService = \Modules\ProviderManagement\Entities\SubscribedService::where('provider_id', $providerId)
                            ->where('service_id', $sId)
                            ->first();
                        if ($subscribedService && $subscribedService->service_price > 0) {
                            $price = $subscribedService->service_price;
                        }
                    }
                }

                // Final fallback
                if ($price <= 0 && isset($service->price) && $service->price > 0) {
                    $price = $service->price;
                }
                ...
                $tax = round((($price * $quantity - $applicableDiscount) * $service['tax']) / 100, 2);
                ...
                $cart->service_id = $sId;
                $cart->tyre_id = $sId == $request->service_id ? ($request['tyre_id'] ?? null) : null;
                $cart->category_id = $currentCategoryId;
                $cart->sub_category_id = $currentSubCategoryId;
                $cart->variant_key = $currentVariantKey;
                $cart->quantity = $quantity;
                $cart->service_cost = $price;
    ...
    public function rebookAddToCart(Request $request): JsonResponse
    {
        ...
        foreach ($booking->detail as $detail) {
            ...
            if ($serviceData) {
                ...
                if (isset($variation)) {
                    DB::transaction(function () use ($detail, $customerUserId, $variation, $provider, $booking, $request, $serviceData) {
                        ...
                        // Tyre Price Logic
                        if (isset($detail->tyre_id) && !empty($detail->tyre_id)) {
                            $tyre = \Modules\ServiceManagement\Entities\Tyre::find($detail->tyre_id);
                            if ($tyre) {
                                $price = $tyre->price;
                            }
                        }

                        if ($provider && empty($detail->tyre_id)) {
                            $customPrice = null;
                            if (isset($variation)) {
                                $customPrice = ProviderServicePrice::where('provider_id', $provider->id)
                            ...
                            if ($customPrice) {
                                ...
                                // Check for service-level override
                                $subscribedService = \Modules\ProviderManagement\Entities\SubscribedService::where('provider_id', $provider->id)
                                    ->where('service_id', $detail->service_id)
                                    ->first();
                                if ($subscribedService && $subscribedService->service_price > 0) {
                                    $price = $subscribedService->service_price;
                        ...
                        $cart->category_id = $serviceData->category_id;
                        $cart->sub_category_id = $serviceData->sub_category_id;
