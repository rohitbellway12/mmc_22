        ...
        if (!is_null($request['booking_id'])) {
            ...
            if (!is_null($request['car_booking_id'])) {
                ...
                if ($carBooking) {
                    ...
                    $carBooking->payment_method = $request['payment_method'];
                    $carBooking->transaction_id = $tran_id;
                    $carBooking->save();

                    // Specific Car Booking Transaction
                    placeCarBookingTransactionForDigitalPayment($carBooking);
                }
            }
    ...
    public static function carBookingPaymentSuccess($data): array
    {
        ...
        if (!is_null($request['car_booking_id'])) {
            ...
            if ($carBooking) {
                ...
                // Mirror payment success into linked regular booking (if exists)
                if (!empty($carBooking->booking_id)) {
                    $linkedBooking = Booking::find($carBooking->booking_id);
                    if ($linkedBooking) {
                        $linkedBooking->is_paid = 1;
                        $linkedBooking->payment_method = $carBooking->payment_method;
                        $linkedBooking->transaction_id = $carBooking->transaction_id;
                        $linkedBooking->save();
                    }
                }
                ...
                if ($admin_user_id) {
                    ...
                    if ($admin_account) {
                        $admin_account->balance_pending += $carBooking->total_amount;
                        $admin_account->save();
                    }
                    ...
                    Transaction::create([
                        ...
                        'debit' => 0,
                        'credit' => $carBooking->total_amount,
