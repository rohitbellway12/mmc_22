    ...
    <div class="main-content">
        <div class="container-fluid">
            <div class="page-title-wrap mb-4">
                <h2 class="page-title text-center">{{ translate('Update Your Car Hire Service') }}</h2>
                <p class="text-center text-muted">{{ translate('Manage your vehicle details, pricing and availability.') }}
                </p>
            </div>
            ...
            <div class="card form-card max-w-800 mx-auto">
                <div class="card-body p-4 p-md-5">
                    <form action="{{ route('provider.car.update', [$car->id]) }}" method="POST"
                        enctype="multipart/form-data" id="car-hire-form">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="step-container active" id="step-1">
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="brand" class="form-control h-45"
                                        value="{{ $car->brand }}" placeholder="{{ translate('Eg "Audi A4"') }}" required>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-6 col-sm-4">
                                    ...
                                    <input type="number" name="manufacture_year" class="form-control h-45"
                                        value="{{ $car->manufacture_year }}" placeholder="2022">
                                </div>
                                <div class="col-6 col-sm-4">
                                    <label class="form-label">{{ translate('Seating Capacity') }}</label>
                                    <input type="number" name="seating_capacity" class="form-control h-45"
                                        value="{{ $car->seating_capacity }}" placeholder="5">
                                </div>
                                <div class="col-sm-4">
                                    ...
                                    <select name="transmission_type" class="form-control h-45">
                                        <option value="Automatic"
                                            ...
                                            {{ translate('Automatic') }}</option>
                                        <option value="Manual" {{ $car->transmission_type == 'Manual' ? 'selected' : '' }}>
                                    ...
                                    <select name="car_type_id" class="form-control h-45" required>
                                        @foreach ($types as $type)
                                            <option value="{{ $type->id }}"
                                                {{ $car->car_type_id == $type->id ? 'selected' : '' }}>{{ $type->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="mt-4 mb-3">
                                ...
                                <p class="small text-muted mb-3">
                                    {{ translate('Existing photos are shown below. Uploading new photos will add them to your collection.') }}
                                </p>
                                        ...
                                        <div class="image-upload-wrapper">
                                            <div class="image-upload-box" id="box_{{ $view }}"
                                                ...
                                                <div class="placeholder" id="placeholder_{{ $view }}"
                                                    style="{{ isset($carImages[$index]) ? 'display:none' : '' }}">
                                                    <i class="material-icons">add_a_photo</i>
                                                    <span>{{ translate('Upload') }}</span>
                                                </div>
                                                <img src="{{ isset($carImages[$index]) ? asset('storage/app/public/car/' . $carImages[$index]) : '' }}"
                                                    id="preview_{{ $view }}"
                                                    style="{{ isset($carImages[$index]) ? 'display:block' : 'display:none' }}">
                                            </div>
                            ...
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        <input type="number" name="hourly_rate" class="form-control h-45"
                                            value="{{ $car->hourly_rate }}" placeholder="0.00" step="0.01" required>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="number" name="security_deposit" class="form-control h-45"
                                        value="{{ $car->security_deposit }}" placeholder="0.00" step="0.01">
                                </div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="postcode" class="form-control h-45"
                                        value="{{ $car->postcode }}" placeholder="{{ translate('Eg "TW6 1AP"') }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ translate('Available For') }}</label>
                                    <select name="available_for" class="form-control h-45">
                                        <option value="Pickup" {{ $car->available_for == 'Pickup' ? 'selected' : '' }}>
                                            {{ translate('Pickup Only') }}</option>
                                        <option value="Delivery"
                                            {{ $car->available_for == 'Delivery' ? 'selected' : '' }}>
                                            {{ translate('Delivery Only') }}</option>
                                        <option value="Both" {{ $car->available_for == 'Both' ? 'selected' : '' }}>
                                            {{ translate('Both (Pickup & Delivery)') }}</option>
                                    </select>
                                </div>
                                <div class="col-12">
                                    <label class="form-label">{{ translate('Collection Address') }}</label>
                                    <textarea name="address" class="form-control" rows="3"
                                        placeholder="{{ translate('Complete address for vehicle collection...') }}">{{ $car->address }}</textarea>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="time" name="available_hours_start" class="form-control h-45"
                                        value="{{ $car->available_hours_start ?? '09:00' }}">
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <input type="time" name="available_hours_end" class="form-control h-45"
                                        value="{{ $car->available_hours_end ?? '18:00' }}">
                                </div>
                            </div>
                        ...
                        <div class="step-container" id="step-3">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <textarea name="terms_conditions" class="form-control" rows="6"
                                    placeholder="{{ translate('State your rules...') }}">{{ $car->terms_conditions }}</textarea>
                            </div>
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label">{{ translate('Vehicle Registration (V5C)') }}</label>
                                    <div class="custom-file mb-1">
                                ...
                                <div class="col-md-6">
                                    <label class="form-label">{{ translate('Address Proof Documents') }}</label>
                                    <div class="custom-file mb-1">
                            ...
                            <div class="d-flex justify-content-between mt-5 pt-3">
                                ...
                                <button type="submit"
                                    class="btn btn--primary px-5 h-45">{{ translate('Update & Save') }}</button>
                            </div>
                        </div>
                    </form>
                </div>
