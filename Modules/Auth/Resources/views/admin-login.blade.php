...
<body>
    ...
    <div class="glass-card">
        ...
        <h1 class="form-title">{{ translate('Admin Sign In') }}</h1>
        <p class="form-subtitle">{{ translate('Premium Automotive Service Management') }}</p>
        ...
        <form action="{{ route('admin.auth.login') }}" method="POST" id="login-form">
            ...
            <div class="form-group-premium">
                ...
                <input type="email" name="email_or_phone" class="form-control input-premium"
                    value="{{ request()->cookie('remember_email') }}" placeholder="{{ translate('Email Address') }}"
                    required id="email">
            </div>
            ...
            <div class="form-group-premium">
                ...
                <input type="password" name="password" class="form-control input-premium"
                    value="{{ request()->cookie('remember_password') }}" placeholder="{{ translate('Password') }}"
            ...
            <button type="submit" class="btn btn-premium"
                id="signInBtn">{{ translate('Login to Dashboard') }}</button>

            <div class="form-footer">
                {{ translate('Accessing Provider Terminal?') }}
                <a href="{{ route('provider.auth.login') }}">{{ translate('Switch to Provider') }}</a>
            </div>
        </form>
    </div>
    ...
    <script>
        ...
        // Duplicate credentials for demo mode
        @if (env('APP_ENV') == 'demo')
            console.log("Demo Mode: admin@admin.com / 12345678");
        @endif
