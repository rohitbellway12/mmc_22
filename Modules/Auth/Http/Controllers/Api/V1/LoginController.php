    ...
    public function providerOtpLogin(Request $request): JsonResponse
{
     ...
     * @return JsonResponse
     */
    public function customerOtpLogin(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
            'phone' => 'nullable|regex:/^([0-9\s\-\+\(\)]*)$/|min:10',
            'email' => 'nullable|email|max:255',
    ...
    return response()->json(
        response_formatter(
            AUTH_LOGIN_200,
            self::authenticate($user, CUSTOMER_PANEL_ACCESS)
        ),
        200
    );
}
          
            $verification_data = DB::table('user_verifications')
                ->where('identity', $identity)
                ->where('identity_type', $identityType)
                ->where(['otp' => $request['otp']])
                ->where('expires_at', '>', now())
