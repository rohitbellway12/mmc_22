...
public function index(Request $request): JsonResponse
{
    ...
    $activeCoupons = $this->coupon
        ->withoutGlobalScope('zone_wise_data')   // ← zone filter bypass
        ->with(['discount', 'coupon_customers'])
        ->whereHas('discount', function ($query) {
            $query->where('promotion_type', 'coupon')
                ->whereDate('start_date', '<=', now())
                ->whereDate('end_date', '>=', now())
    ...
    return response()->json(response_formatter(DEFAULT_200, $activeCoupons), 200);
}
    /**
     * Display a listing of the resource.
     * @param Request $request
...
// // If cart is empty, return all active coupons
...
//     return response()->json(response_formatter(DEFAULT_200, $couponQuery), 200);
// }
...
public function applicable(Request $request)
{
    ...
    $couponQuery = $this->coupon
        ->withoutGlobalScope('zone_wise_data')   // ← zone scope bypass
        ->with(['discount.discount_types'])
        ->ofStatus(1)
        ->whereHas('discount', function ($query) {
            $query->where(['promotion_type' => 'coupon'])
