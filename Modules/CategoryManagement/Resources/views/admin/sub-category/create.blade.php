                                        ...
                                        <div class="col-lg-8 mb-5 mb-lg-0">
                                            <div class="d-flex flex-column">
                                                <select class="js-select theme-input-style w-100" name="parent_id"
                                                    id="category_selector" required>
                                                    <option value="" selected disabled>
                                                        {{ translate('Select_Category_Name') }}</option>
                                                ...
                                                @if ($language)
                                                    ...
                                                    @foreach ($language?->live_values as $index => $lang)
                                                        <div class="lang-form d-none" id="{{ $lang['code'] }}-form">
                                                            ...
                                                            <div class="form-floating mb-30">
                                                                <textarea type="text" name="short_description[]" class="form-control resize-none">{{ old('short_description.' . ($index + 1)) }}</textarea>
                                                ...
                                                @else
                                                    ...
                                                    <div class="form-floating form-floating__icon mb-30">
                                                        <textarea type="text" name="short_description[]" class="form-control resize-none" required></textarea>
                                                        <label>{{ translate('short_description') }}
                                                            ({{ translate('default') }})</label>
                                                    </div>
                                        ...
                                        <div class="col-lg-4">
                                            <div class="d-flex  gap-3 gap-xl-5">
                                                <p class="opacity-75 max-w220">
                                                    {{ translate('image_format_-_jpg,_png,_jpeg,_gif_image
                                                                                                                                                                                                                                                                size_-_
                                                                                                                                                                                                                                                                maximum_size_2_MB_Image_Ratio_-_1:1') }}
                                                </p>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive">
                                        <table id="example" class="table align-middle">
                                            <thead class="text-nowrap title-color">
                                                <tr>
                                                    <th class="title-color">{{ translate('SL') }}</th>
                                                    <th class="title-color">{{ translate('name') }}</th>
                                                    <th class="title-color">{{ translate('parent_category') }}</th>
                                                    <th class="title-color">{{ translate('service_count') }}</th>
                                                    @can('category_manage_status')
                                                        <th class="title-color">{{ translate('status') }}</th>
                                                    @endcan
                                                    @canany(['category_delete', 'category_update'])
                                                        <th class="title-color">{{ translate('action') }}</th>
                                                    @endcan
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($subCategories as $key => $category)
                                                    <tr>
                                                        <td class="title-color">{{ $subCategories->firstitem() + $key }}
                                                        </td>
                                                        <td class="text-capitalize title-color">{{ $category->name }}</td>
                                                        <td class="title-color">
                                                            {{ $category->parent->name ?? translate('not_found') }}</td>
                                                        <td class="title-color">{{ $category->services_count }}</td>
                                                        @can('category_manage_status')
                                                            <td>
