...
Route::group(['prefix' => 'provider', 'as' => 'provider.', 'namespace' => 'Web\Provider', 'middleware' => ['provider']], function () {
    Route::group(['prefix' => 'service', 'as' => 'service.'], function () {
        ...
        Route::put('update-capabilities', 'ServiceController@updateCapabilities')->name('update-capabilities');
        Route::put('update-service-details', 'ServiceController@updateServiceDetails')->name('update-service-details');
        Route::post('update-emergency-settings', 'ServiceController@updateEmergencySettings')->name('update-emergency-settings');
        Route::any('detail/{id}', 'ServiceController@show')->name('detail');
        Route::post('review-reply', 'ServiceController@reviewReply')->name('review.reply');
    });
    Route::group(['prefix' => 'tyre', 'as' => 'tyre.'], function () {
        Route::get('index', 'ProviderTyreController@index')->name('index');
        Route::get('create', 'ProviderTyreController@create')->name('create');
        Route::post('store', 'ProviderTyreController@store')->name('store');
        Route::get('edit/{id}', 'ProviderTyreController@edit')->name('edit');
        Route::post('update/{id}', 'ProviderTyreController@update')->name('update');
        Route::any('status-update/{id}', 'ProviderTyreController@statusUpdate')->name('status-update');
        Route::delete('delete/{id}', 'ProviderTyreController@destroy')->name('delete');
    });
});
