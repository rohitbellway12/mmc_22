...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('public/assets/provider-module') }}/css/custom.css" />
    @stack('css_or_js')
    <style>
        ...
        @if (env('APP_ENV') == 'demo')
            <div class="alert alert--message-2 alert-dismissible fade show" id="demo-reset-warning">
                <img width="28" class="align-self-start"
                    src="{{ asset('public/assets/admin-module/img/info-2.png') }}" alt="">
    ...
    <script>
            ...
            $('#searchForm input[name="search"]').keyup(function() {
                ...
                if (searchKeyword.length >= 2) {
                    $('#searchResults').empty().html(
                        '<div class="text-center text-muted py-5">{{ translate('Searching....') }}</div>'
                    );
                    $.ajax({
                        type: 'POST',
        ...
        $(document).ready(function() {
            $("#staticBackdrop").on("shown.bs.modal", function() {
                ...
                $('#searchResults').html(
                    '<div class="text-center text-muted py-5">{{ translate('Loading recent searches') }}...</div>'
                );
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        console.error(xhr.responseText);
                        $('#searchResults').html(
                            '<div class="text-center text-muted py-5">{{ translate('Error loading recent searches') }}.</div>'
                        );
                    }
                });
            });
                ...
                @if (!empty($fcm_credentials))
                        ...
                        messaging
                            ...
                            .then(function(token) {
                                subscribeTokenToBackend(token,
                                    'demandium_provider_{{ auth()->user()->provider->zone_id }}_{{ auth()->user()->provider->id }}_booking_message'
                                );
                                @if ($serviceAtProviderPlace)
                                    @if (in_array('customer', $serviceLocations))
                                        subscribeTokenToBackend(token,
                                            'demandium_provider_{{ auth()->user()->provider->zone_id }}_customer_booking_message'
                                        );
                                    @endif
                                    @if (in_array('provider', $serviceLocations))
                                        subscribeTokenToBackend(token,
                                            'demandium_provider_{{ auth()->user()->provider->zone_id }}_provider_booking_message'
                                        );
                                    @endif
                                @else
                                    subscribeTokenToBackend(token,
                    ...
                    messaging.onMessage(function(payload) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                playAudio(false);
                                checkBooking(payload.data
                                    .booking_id
                                    ); // In firebase, we might not have the count easily, but passing something non-null helps. 
                                // Actually, payload.data might have something. But for now, just ensure it's not NaN.
                                location.href =
                                    "{{ route('provider.booking.list') }}?booking_status=pending&service_type=all";
                            } else if (result.dismiss === 'close') {
                                playAudio(false);
                                checkBooking(payload.data.booking_id);
                                location.href =
                                    "{{ route('provider.booking.list') }}?booking_status=pending&service_type=all";
                            }
                        })
