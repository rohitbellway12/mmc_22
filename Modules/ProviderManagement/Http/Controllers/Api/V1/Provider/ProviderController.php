...
class ProviderController extends Controller
{
    ...
    public function forgotPassword(Request $request): JsonResponse
    {
        ...
        $identity = $request['phone_or_email'];
        DB::table('password_resets')
            ->where(function ($query) use ($identity) {
                $query->where('phone', $identity)->orWhere('email', $identity);
            })
            ->delete();
        ...
        $token = env('APP_ENV') != 'live' ? '123456' : rand(100000, 999999);
        ...
        $identityType = filter_var($identity, FILTER_VALIDATE_EMAIL) ? 'email' : 'phone';
        $method = business_config('forget_password_verification_method', 'business_information')?->live_values;
        $deliveryMethod = $identityType === 'email' ? 'email' : ($method ?? 'phone');
        ...
        if ($deliveryMethod == 'phone') {
            ...
            if ($emailStatus) {
                try {
                    Mail::to($customer->email)->send(new OTPMail($token));
