...
class AccountController extends Controller
{
     ...
     * @return JsonResponse
     */
    public function payToAdmin(Request $request): JsonResponse
    {
        $validator = Validator::make($request->all(), [
        ...
        if ($provider) {
            ...
            if ($customer->account->account_payable > $customer->account->account_receivable) {
                $amount = $customer->account->account_payable - $customer->account->account_receivable;
                ...
                if ($minPayableAmount > 0 && $amount < $minPayableAmount) {
                    return response()->json(response_formatter(DEFAULT_400, null, [["error_code" => "amount", "message" => translate('Minimum payable amount is: ') . $minPayableAmount]]), 400);
                }
                ...
                return response()->json(response_formatter(DEFAULT_400, null, [["error_code" => "amount", "message" => translate('Invalid Amount or No Dues')]]), 400);
            }
        }
