...
class ProviderController extends Controller
{
    ...
    public function getUpdatedData(Request $request): JsonResponse
    {
        $subscribed = $this->subscribed_sub_categories->where('provider_id', $request->user()->provider->id)
            ...
            ->with('service')
            ->get()
            ->pluck('service.sub_category_id')
            ->unique()
            ->toArray();

        $booking = $this->booking
            ->where('zone_id', $request->user()->provider->zone_id)
            ->where(function ($query) use ($subscribed, $request) {
                $query->whereIn('sub_category_id', $subscribed)
                    ->whereNull('provider_id')
                    ->orWhere('provider_id', $request->user()->provider->id);
            })
    ...
    public function dashboard(Request $request, Transaction $transaction, SubscribedService $subscribedService, Serviceman $serviceman): Renderable
    {
        ...
        //recent_bookings
        // Fetch sub_category_ids from the subscribed services
        $subCategoryIds = $this->subscribed_service->where('provider_id', $request->user()->provider->id)
            ->ofSubscription(1)
            ->with('service')
            ->get()
            ->pluck('service.sub_category_id')
        ...
        $recent_bookings = $this->booking->with([
        ...
        $subCategories = $subCategoryIds; // Reuse the IDs fetched above
    ...
    public function subscribedSubCategories(Request $request): Renderable
    {
        ...
        $subscribedSubCategories = $this->subscribedService->where('provider_id', $request->user()->provider->id)
            ...
            ->where(function ($query) use ($keys) {
                foreach ($keys as $key) {
                    $query->orWhereHas('service', function ($query) use ($key) {
    ...
    public function termsAndConditions(): Renderable
    {
        ...
        if (!$terms) {
            $terms = translate('Any faults or issues discovered that are beyond the scope of the initial service booking will incur additional charges. The customer will be informed before any such work is performed.');
        }
        return view('providermanagement::provider.settings.terms-conditions', compact('terms'));
    }

    public function updateTermsAndConditions(Request $request): RedirectResponse
    {
        $request->validate([
    ...
    public function aboutUs(): Renderable
    {
        ...
        return view('providermanagement::provider.settings.about-us', compact('aboutUs'));
    }
