...
class BookingReportController extends Controller
{
    ...
    public function getBookingReport(Request $request): Renderable
    {
        ...
        // Car booking list (provider car hire) - shown in the same report page
        $providerId = $request->user()->provider->id;
        $filteredCarBookings = CarBooking::with(['car', 'user'])
            ->whereHas('car', function ($query) use ($providerId) {
                $query->where('provider_id', $providerId);
            })
            ->when($request->filled('booking_status') && $request['booking_status'] !== 'all', function ($query) use ($request) {
            ...
            ->when($request->filled('date_range') && $request['date_range'] === 'custom_date', function ($query) use ($request) {
                return $query->whereBetween('created_at', [Carbon::parse($request['from'])->startOfDay(), Carbon::parse($request['to'])->endOfDay()]);
            })
            ->when($request->filled('date_range') && $request['date_range'] !== 'custom_date', function ($query) use ($request) {
        ...
        return view('providermanagement::provider.report.booking', compact('zones', 'categories', 'subCategories', 'filteredBookings', 'filteredCarBookings', 'bookingsCount', 'bookingAmount', 'chartData', 'queryParams'));
    }
